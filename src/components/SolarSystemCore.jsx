...
const Planets = ({ count = 14 }) => {
    ...
    const { camera } = useThree(); // For click handling
    const { handleFocus } = useCamera(); // For focusing

    // Load planet texture - replace with your actual texture path
    const planetTexture = useLoader(TextureLoader, '/textures/earth_daymap.jpg'); 
    // You can create an array of texture paths if you want different textures for different planets,
    // but that would require a more complex material strategy with instancing (e.g., texture array or custom shader).
    ...
    const createBodyData = (options = {}) => {
        ...
        // The scale from createBodyData will be used by InstancedRigidBodies for each instance.
        // The args in sphereGeometry will define the base size (radius 1 typically).
        const scale = scaleOverride !== undefined ? scaleOverride : 0.5 + Math.random() * 1.5;
        const type = typeOverride || 'Planet';
        return { key, position, linearVelocity, scale, userData: { type, key } }; // Pass scale here
    };

    const initialInstances = useMemo(() => {
        const bodies = [];
        if (count > 0) {
            bodies.push(createBodyData({ typeOverride: 'Moon', scaleOverride: 0.5 })); // Example scale
            if (count > 1) {
                bodies.push(createBodyData({ typeOverride: 'Earth', scaleOverride: 1.2 })); // Example scale
            }
            const remainingPlanetsCount = Math.max(0, count - bodies.length);
            for (let i = 0; i < remainingPlanetsCount; i++) {
                bodies.push(createBodyData({ typeOverride: 'Planet' })); // Default scale will be random
            }
        }
        return bodies;
    }, [count]);
        ...
        if (planetsRef.current) {
           // setPlanetCount(planetsRef.current.length); // Update count if needed elsewhere
            planetsRef.current.forEach((planetBody) => {
                // Ensure angvel is an object {x, y, z}
                planetBody.setAngvel({ x: 0, y: Math.random() - 0.5, z: 0 }, true); 
            });
        }
    }, [initialInstances]); 
    ...
    const handleCollision = ({ manifold, target, other }) => {
        ...
        if(!targetRigidBody.userData || !otherRigidBody.userData) return;

        const targetMass = targetRigidBody.mass();
        const otherMass = otherRigidBody.mass();

        if (otherMass > targetMass && targetMass > 0) { 
            ...
            const newBodyData = createBodyData({ respawn: true, typeOverride: 'Planet' }); 

            targetRigidBody.userData.key = newBodyData.key; 
    ...
    // Click handler for instanced meshes
    const onInstanceClick = (event) => {
        event.stopPropagation(); // Important to stop propagation
        if (event.instanceId !== undefined && handleFocus) {
            // Pass the InstancedRigidBodies ref (planetsRef.current) as the object
            // and the specific instanceId to the handleFocus function.
            handleFocus({ object: planetsRef.current, instanceId: event.instanceId });
        }
    };
    ...
    return (
        <InstancedRigidBodies 
            ...
            // onClick={onInstanceClick} // Note: onClick on InstancedRigidBodies directly might not work as expected
                                      // for individual instances. Click events are usually handled by casting a ray
                                      // or by attaching event handlers to the geometry/material if supported for instancing.
                                      // For Rapier, onCollisionEnter is physics-based. For clicks, often need to add a <group onClick={...}> around geometry.
        >
            {/* DIAGNOSTIC: Use very simple geometry and material */}
            <boxGeometry args={[1, 1, 1]} /> 
            <meshBasicMaterial color="red" />
            {/* <sphereGeometry args={[1, 32, 32]} /> */}
            {/* <meshStandardMaterial map={planetTexture} roughness={0.8} metalness={0.2} /> */}
            
            {/* Add onClick to a group wrapping geometry if you need individual instance clicks for focus */}
            {/* Example for click handling (more advanced, might need adjustments): */}
            {/* 
            <group onClick={onInstanceClick}>
                <sphereGeometry args={[1, 32, 32]} /> 
                <meshStandardMaterial map={planetTexture} roughness={0.8} metalness={0.2} />
            </group>
            */}
...
const Scene = () => {
    useGravity(); // Apply gravity to all dynamic bodies

    return (
        <>
            <Stars />
            <Sun />
            {/* <Planets /> */} {/* Comment out Planets for diagnostic purposes */}
